...
# from models.network_unet5 import Unet5
from models.network_ifnet import IFNet
from models.loss_stage1 import loss_fusion
...
class ModelPlain(ModelBase):
    ...
    def define_loss(self):
        ...
        elif G_lossfn_type == 'vif1':
            from models.loss_vif1 import fusion_loss_vif
            self.G_lossfn = fusion_loss_vif().to(self.device)    
        ...
        elif G_lossfn_type == 'emma':            
            from models.loss_emma import fusion_loss_emma
            self.G_lossfn = fusion_loss_emma().to(self.device)    
        else:
    ...
    def define_optimizer(self): 
        ...
        for k, v in IFmodel.named_parameters():
            if v.requires_grad:
                G_optim_params.append(v)
            else:
                print('Params [{:s}] will not optimize.'.format(k))        
    ...
    def netG_forward(self, phase='test'):
        self.E = self.netG(self.A, self.B)#['fused']
        self.I,self.V = IFmodel(self.E)
    ...
    def optimize_parameters(self, current_step):
        ...
        fi_loss = alpha1*loss(self.I,self.A) # A is IR
        fv_loss = alpha2*loss(self.V,self.B) # B is VI
        G_loss = fi_loss+fv_loss
        G_loss.backward()
